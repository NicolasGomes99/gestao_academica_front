...
const RenderCampo: React.FC<RenderCampoProps> = ({ campo: e, dados, setDados, photoPreview, setPhotoPreview, expandedDocUrl, setExpandedDocUrl, expandedDocType, setExpandedDocType, alterarInput, alterarRatio, onClickFuncao }) => {
  ...
  return (
    <div className={`flex flex-col ${e.colSpan ?? ''}`}>
      ...
      {e.tipo === 'button' && (
        <button type="button" name={e.chave} onClick={onClickFuncao ? () => { onClickFuncao(); } : undefined} className="bg-extra-150 hover:bg-extra-50 text-white px-4 py-2 rounded text-body-medium disabled:opacity-50 mt-6 mb-0" disabled={!!e.bloqueado}>{e.nome}</button>
      )}
